...
int main(int argc , char ** argv){
    ...
    float f_ref[6] = { 0.0, 0.0, 0.6, 0, 0, 0 }; // the force(N) that xArm will apply to the environment
    ...
    // set  robot  at  starting  point  like  abive the teeth  to don't hit teeth when he make  inner cleening  
    float x = 357.4;
    float y = 225.1;
    float z = 479.7;
    float roll_raw  =deg2rad(143.9) ;
    float pitch_raw = deg2rad(-13.8);
    float yaw_raw   = deg2rad(109.7); 
    ...
    // set  robot  at  starting  point where it should hit teeth  
    x = 337.5;
    y = 207.6;
    z =470.6 ;
    roll_raw  =deg2rad(143.9) ;
    pitch_raw = deg2rad(-13.8);
    yaw_raw   = deg2rad(109.7); 
        // Fill pose array
    float second_pose[6] = {
            x, y, z,
            roll_raw,
   ...
   std::cout<<"robot is ready to move now\n";
    ...
    for (size_t i = 0; i < csv_data.size(); ++i) {
        ...
        x = static_cast<float>(csv_data[i][0]);
        y = static_cast<float>(csv_data[i][1]);
        z = static_cast<float>(csv_data[i][2]-1.5f);
        // x= 373.9;
        // y= 224.5 ;
        // z=441.9;
